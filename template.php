...
function boilerplate_preprocess_page(&$vars, $hook) {
  ...
  // Cargamos JQuery y Anythingslider unicamente en la home, para evitar conflictos en
  // las paginas de administracion de views y con el ckeditor
  //if (drupal_is_front_page()) { //(arg(0) != 'admin') {
  //  drupal_add_js('/sites/all/themes/boilerplate/js/libs/jquery-1.11.0.min.js');
  //  drupal_add_js('/sites/all/themes/boilerplate/js/libs/jquery.anythingslider.min.js');
  //}
}
...
/*
function boilerplate_preprocess_field(&$variables, $hook) {
...
function boilerplate_breadcrumb($variables) {
  ...
  if ($show_breadcrumb == 'yes' || ($show_breadcrumb == 'admin' && arg(0) == 'admin')) {
    ...
    if (!empty($breadcrumb)) {
      ...
      $result = implode($breadcrumb_separator, $breadcrumb);
...
*/
function boilerplate_preprocess_search_block_form(&$vars) {
  $vars['search_form'] = str_replace('value=""', 'placeholder="Introduce un criterio de búsqueda"', $vars['search_form']);
}
